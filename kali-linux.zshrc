...
if [ -n "$force_color_prompt" ]; then
 ...
 configure_prompt() {
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
#            PROMPT=$'%F{%(#.blue.white)}┌──${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.white)}%n'$prompt_symbol$'%m%b%F{%(#.white.white)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.white.white)}]\n└─%B%(#.%F{red}#.%F{cyan}$)%b%F{reset} '
           # Right-side prompt with exit codes and background processes
           #RPROMPT=$'%(?.. %? %F{red}%B⨯%b%F{reset})%(1j. %j %F{yellow}%B⚙%b%F{reset}.)'
	   PROMPT=' %B%F{cyan}┌─ [%B%F{reset}%B%F{white}%(6~.%-1~/…/%4~.%5~)%B%F{cyan}]'$'\n └─── > %b%f'

           # Right-side prompt with exit codes and background processes
           #RPROMPT=$'%(?.. %? %F{red}%B⨯%b%F{reset})%(1j. %j %F{yellow}%B⚙%b%F{reset}.)'
            ;;
        oneline)
           PROMPT=$'${debian_chroot:+($debian_chroot)}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))}%B%F{%(#.red.cyan)}%n@%m%b%F{reset}:%B%F{%(#.cyan.white)}%~%b%F{reset}%(#.#.$) '
           RPROMPT=
           ;;
        backtrack)
            PROMPT=$'${debian_chroot:+($debian_chroot)}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))}%B%F{red}%n@%m%b%F{reset}:%B%F{cyan}%~%b%F{reset}%(#.#.$) '
            RPROMPT=
            ;;
    esac
    unset prompt_symbol
